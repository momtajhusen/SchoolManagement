    // Teacher Image Crope   
    $('#drive_img_input').change(function(event) 
    {
        var $modal = $('#driver-modal');
        var image = document.getElementById('driver_sample_image');
        var cropper;
        ...
        if (files && files.length > 0) {
            ...
            $('#driver_img_preview').attr("src",  "http://bit.ly/3IUenmf");
        }
        ...
        $modal.on('shown.bs.modal', function() {
          ...
          $("#driver-model-cancle").on("click", function() {
            $('#drive_img_input').val("");
            $('#driver_img_preview').attr("src",  "http://bit.ly/3IUenmf");
          });
          
    
        $("#driver-crop").click(function() {
            ...
            canvas.toBlob(function(blob) {
                ...
                reader.onloadend = function() {
                    ...
                    $modal.modal('hide');
                    $('#driver_img_preview').attr('src', base64data);
                    ...
                    if (cropped) {
                        ...
                        if (localStorage.getItem("driver_register") === null) {
                            var image_name = "driver-" + new Date().toISOString().replace(/[-:.TZ]/g, '') + Math.floor(Math.random() * 10000);
                            localStorage.setItem("driver_register", image_name);
                        } else {
                            var image_name = localStorage.getItem("driver_register");
                        }
