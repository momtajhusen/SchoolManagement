...
class FeePaymenthMondthyController extends Controller
{
            ...
            if(Student::where('id', $student_id)->where("admission_status", "admit")->first())
            {
                    ...
                    for ($i = $start_month; $i <= 11; $i++) {
                        ...
                        /////////// Start Check Hostel Deposit Fee ///////////
                            ...
                            if ($joining_months) {
                                ...
                                if($hosteldeposit_months_array[$i] == 1)
                                {
                                    $hosteldeposit_fee =  $FeestractureOnetime->hostel_deposit;
                                }
                                else{
                                    $hosteldeposit_fee = 0; 
                                }
                                ...
                                // Start Check Discount Exception 
                                    $DiscountExceptions = DiscountExceptions::where('st_id', $student_id)->where("fee_type", "Hostel Deposit")->first();
                                    if($DiscountExceptions){
                                        $hosteldepositDisc = $DiscountExceptions->dis; 
                                        $hosteldepositDiscAmount = ($hosteldeposit_fee * $hosteldepositDisc) / 100;
                                        $hosteldeposit_fee = $hosteldeposit_fee - $hosteldepositDiscAmount;
                                    }
                                // End Check Discount Exception

                            if ($hosteldeposit_fee != 0) {
                                $feeMonthly['month_'.$i] += $hosteldeposit_fee;
                                $TotalFee += $hosteldeposit_fee;
                                $MonthFeeGenerate += $hosteldeposit_fee;
                            }
                        /////////// End Check Hostel Deposit Fee  ///////////
                ...
                ///////////////////// End Total Feee Retrive ///////////////////////////
                    ...
                    /////////// Start Retrive PaymentHistory Data ///////////
                        ...
                        $paymentresetStatus = $resetStatus->reset_status ?? 'payment';

                        // Query the database to match the year part of the date

                        $paymentHistories = PaymentHistory::where("student_id", $student_id)
                        ->where('pay_date', 'LIKE', LaravelNepaliDate::toEnglishYear($year).'%')
                        ->orderBy('id', 'desc')
                        ->get();

                        // echo LaravelNepaliDate::toEnglishYear($year);

                        // Update pay_date for each record in memory without saving to the database
                    ...
                    return response(array("data" => $this->allData, "ExceptionFreeFee"=>$ExceptionFreeFee, "ExceptionDiscountFee"=>$ExceptionDiscountFee, "parent_data"=> $parent_data, 'BackYear' => $YearFee,  'feeMonthly' => $feeMonthly, 'PaymentFee' => $PaymentFee, 'FeeDiscount' => $FeeDiscount, 'FeeFree' => $FeeFree, 'DuesAmount' => $DuesAmount, "PaymentHistory" => $paymentHistories, "paymentresetStatus" => $paymentresetStatus), 200);
                } else {
                ...
                /////////// Start Check Hostel Deposit Fee ///////////
                        ...
                        // Start exam fee month check than add month
                            ...
                            for ($i = $start_month; $i < $length; $i++) {
                                ...
                                if ($joining_months) {
                                    $hosteldeposit_months_array = json_decode($joining_months->hostel_deposit, true);
                                    if ($hosteldeposit_months_array[$numerMonth] == 1) {
        ...
        if(Student::where('id', $student_id)->first()){
            ...
            if ($DuesAmountData) 
            {
                ...
                if (!is_null($DuesAmountData->$month) || $DuesAmountData->$month != 0) {
                    ...
                    /////////// Start Check Hostel Deposit ///////////
                        // Start hostel_deposit joining month check than add amount
                        if ($joining_months) {
                            $hostel_deposit_months_array = json_decode($joining_months->hostel_deposit, true);
                            if($hostel_deposit_months_array[$numerMonth] == 1)
                            {
                                $hostel_deposit =  $FeestractureOnetime->hostel_deposit;
                            }
                            else{
                                $hostel_deposit = 0; 
                            }
                            ...
                            // Start Check Discount Exception 
                                ...
                                // if($DiscountExceptions){
                                //     $saraswatiDisc = $DiscountExceptions->dis; 
                                //     $examDiscAmount = ($hostel_deposit * $saraswatiDisc) / 100;
                                //     $hostel_deposit = $hostel_deposit - $examDiscAmount;
                                // }
                            // End Check Discount Exception
                
                        if ($hostel_deposit != 0) {
                            $FeeTypeWithAmount["Hostel Deposit"] = $hostel_deposit;
                        }
                    /////////// End Check Hostel Deposit///////////
                ...
                /////////// Start Check Hostel Deposit Fee ///////////
                        ...
                        //// Now Exam Fee calculation with Joining month  ////
                            if($hosteldepositAmount * $hosteldeposit_join_month != 0){
                                ...
                                $FeeTypeWithAmount["Hostel Deposit"] = $hosteldepositFinalAmount;
                            }
                            else{
                                unset($FeeTypeWithAmount["Hostel Deposit"]); 
                            }
        ...
        try {
           $Student = Student::where('id', $request->input('student_id'))->first();
            ...
            { 
                ...
                $hostelDepositArray = $request->input('hostelDepositArray', []);
                ...
                $serializedHostelDepositArray = json_encode($hostelDepositArray);
                ...
                $serializedStartAdmissionArray = json_encode($admissionStartMonthsArray);
                    ...
                    if ($joinLeaveEntry) {
                        ...
                        JoinleaveDates::create([
                            ...
                            'hostel_deposit' => $serializedHostelDepositArray,
                ...
                ///////////////////// Start Total Feee Retrive ///////////////////////////
                    ...
                    for ($i = $start_month; $i <= 11; $i++) {
                        ...
                        /////////// Start Check Hostel Deposit ///////////
                            ...
                            if ($joining_months) {
                                $hostel_deposit_months_array = json_decode($joining_months->hostel_deposit, true);
                                if($hostel_deposit_months_array[$i] == 1)
                                {
                                    $hostel_deposit =  $FeestractureOnetime->hostel_deposit;
                                }
                                else{
                                ...
                                // Start Check Discount Exception 
                                    $DiscountExceptions = DiscountExceptions::where('st_id', $st_id)->where("fee_type", "Hostel Deposit")->first();
                                    if($DiscountExceptions){
                                        $hostelDepositiDisc = $DiscountExceptions->dis; 
                                        $hpDiscAmount = ($hostel_deposit * $hostelDepositiDisc) / 100;
                                        $hostel_deposit = $hostel_deposit - $hpDiscAmount;
                                    }
                                // End Check Discount Exception
                    
                            if ($hostel_deposit != 0) {
                                $TotalFee += $hostel_deposit;
                                $MonthFeeGenerate += $hostel_deposit;
                            }
                        /////////// End Check Hostel Deposit ///////////
    ...
    public function GetjoinMonth(Request $request)
    {
        ...
        if ($data) {
            ...
            $hostelDepositArray = json_decode($data->hostel_deposit);
            ...
            $responseData = [
                ...
                'hostelDepositArray' => $hostelDepositArray,
