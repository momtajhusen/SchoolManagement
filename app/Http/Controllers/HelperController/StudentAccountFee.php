...
class StudentAccountFee extends Controller
{
    ...
    public static function feePaidMonthStatus($year, $student_data)
    {
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            $total_paid = 0;
            $total_disc = 0;
            $unFeeSet = true; // Assume fee not set until proven otherwise
            ...
            foreach ($student_data as $student) {
                ...
                // Check if any student has fee set for this month
                if ($status_fee > 0) {
                    $unFeeSet = false; // Set to false if fee is set for any student
                }
            }
    ...
    /////////////////////////////// Start Old Account Fee Set /////////////////////////
        ...
        public static function StudentFeeMonthYear(Request $request){
            $pr_id = $request->pr_id;
        
            $student_data = Student::where("parents_id", $pr_id)->get();
        
            // student fee year 
            $MonthFeeYear = [];
            // Assuming $student_data contains the table data
            foreach ($student_data as $student) {
                $StudentsFeeMonth = StudentsFeeMonth::where('st_id', $student->id)->get();
                
                // Loop through each row to extract the year
                foreach ($StudentsFeeMonth as $record) {
                    // Extract the year from the record and add it to the array
                    $MonthFeeYear[] = $record->year;
                }
            }
            // Remove duplicate years
            $MonthFeeYear = array_unique($MonthFeeYear);
        
            return response()->json(['MonthFeeYear'=>$MonthFeeYear], 200);
        }

        

        public static function StudentAdmissionYear(Request $request){
            $st_id = $request->st_id;
            ...
            $student_data = Student::where("id", $st_id)->first();
        
            $admission_date = LaravelNepaliDate::from($student_data->admission_date)->toNepaliDate();
        
            // Extracting the year from the admission date
            $admission_year = date('Y', strtotime($admission_date));

            // Initialize an empty array to store years
            $years_between = [];
        
            // Loop through the years between current year and admission year in descending order
            for ($year = $current_year; $year >= $admission_year; $year--) {
                $years_between[] = $year;
            }
        
            // Now $years_between array contains all years between current year and admission year in descending order
            return response()->json(['YearsBetween' => $years_between], 200);
        }
