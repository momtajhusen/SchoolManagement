...
class EmployeesSalariesController extends Controller
{
    public function AddSalary(Request $request)
    {
        try {
            ...
            $EmployeesSalaries->salary_date  = $request->input("salary_join_date");
            ...
            StaffAttendance::where('percent', '0')->delete();
              ...
              // Applly BonusSsfApplyEmp
                ...
                foreach($EmpolyAttendance as $attendance){
                        ...
                        $BonusAmount = 0;
                        if($ba == "true"){
                            if ($AttendenPercentage == $bouns_attend) {
                        ...
                        if($ls == "true"){
                            if ($AttendenPercentage <= $leave_per) {
                        ...
                        if($department_role == "Teacher"){
                            $StaffAttendance = TeacherMonthsAttendance::where('emp_id', $teacherSelect)->where('year', $attendanceYear)->where('month', $AttendencMonth)->first();
                        }
                        else{
                            $StaffAttendance = StaffAttendance::where('emp_id', $teacherSelect)->where('year', $attendanceYear)->where('month', $AttendencMonth)->first();
                        }
