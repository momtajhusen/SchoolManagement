...
class StudentsFeePayment extends Controller
{
            ...
            if ($parent_data) 
            {
                ...
                foreach ($student_data as $student) {
                    ...
                    if ($studentFeeStructure) {
                        ...
                        // Fetch paid fee data
                        $studentPaidFeeStructure = StudentsFeePaid::where('year', $year)->where('st_id', $student->id)->first();
                
                        // Fetch fee discount data
                        $studentFeeDiscStructure = StudentsFeeDisc::where('year', $year)->where('st_id', $student->id)->first();
                
                        // Store fee data for each month
                        for ($i = 0; $i < 12; $i++) {
                            $studentFeeStructure->{'paid_month_' . $i} = $studentPaidFeeStructure->{'month_' . $i} ?? '0.00';
                            $studentFeeStructure->{'disc_month_' . $i} = $studentFeeDiscStructure->{'month_' . $i} ?? '0.00';
                        }
                ...
                // $StudentPaidMonthFee = [];
                // foreach ($student_data as $student) {
                //     $studentFeeStructure = StudentsFeePaid::where('year', $year)->where('st_id', $student->id)->first();
                //     if ($studentFeeStructure) {
                //         $student_name = ($student->first_name ?? '') . ' ' . ($student->middle_name ?? ''). ' ' . ($student->last_name ?? '');
                //         $studentFeeStructure->student_name = $student_name;
                //         $StudentPaidMonthFee[] = $studentFeeStructure;
                //     }
                // }

                // $StudentsFeeDisc = [];
                // foreach ($student_data as $student) {
                //     $studentFeeStructure = StudentsFeeDisc::where('year', $year)->where('st_id', $student->id)->first();
                //     if ($studentFeeStructure) {
                //         $student_name = ($student->first_name ?? '') . ' ' . ($student->middle_name ?? ''). ' ' . ($student->last_name ?? '');
                //         $studentFeeStructure->student_name = $student_name;
                //         $StudentsFeeDisc[] = $studentFeeStructure;
                //     }
                // }
    ...
    public function StudentFeePaid(Request $request)
    {
         // Validation rules
            ...
            try {
                ...
                $pay_date = LaravelNepaliDate::from($request->pay_date)->toEnglishDate();
                ...
                ////////////////////// Start History Save //////////////////////
                    ...
                    $StudentsFeePaidHistory->processed_by = session('user_name') ?? '';
                    ...
                    foreach ($st_id_array as $st_id) {
                        ...
                        if ($fee_details) {
                            ...
                            foreach ($pay_month_array as $pay_month) {
                                ...
                                $paid = $student_paid_record->$pay_month;
                                ...
                                $student_paid_record->$pay_month  = $fee-$paid + $student_paid_record->$pay_month - $disc;
                            }
    ...
    public function StudentFeePaidHistory(Request $request) {
        ...
        $StudentsFeePaidHistory = StudentsFeePaidHistory::where('fee_year', $year)
            ->where('pr_id', $pr_id)
            ->orderBy('id', 'desc')
            ->get();
    
        foreach ($StudentsFeePaidHistory as $history) {
            $history->pay_date = LaravelNepaliDate::from($history->pay_date)->toNepaliDate();
        }
    ...
    public function StudentFeeInvoiceData(Request $request){
        try {
            ...
            $hs_id = $StudentsFeePaidHistory->id;
            ...
            $pay_date = LaravelNepaliDate::from($StudentsFeePaidHistory->pay_date)->toNepaliDate();
    ...
    public function StudentFeeDuesList(Request $request) {
        try {
            ...
            // Chunk students processing to avoid memory overflow and timeouts
            $response_data = array();
            $unique_parent_ids = array(); // Array to store unique parent IDs
            
            Student::chunk(100, function($students) use (&$response_data, &$unique_parent_ids, $month_array, $fee_year) {
                foreach ($students as $student) {
