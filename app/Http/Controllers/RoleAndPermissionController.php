...
class RoleAndPermissionController extends Controller
{
    ...
    public function AddSuperAdmin(Request $request)
    {
        try {
            ...
            $validatedData = $request->validate([
                ...
                'number' => 'required|numeric|unique:role_and_permission_users,number',
                'email' => 'required|email|max:255|unique:role_and_permission_users,email',
                'password' => 'required',
            ]);

            // Create a new SuperAdmin record
            $superAdmin = RoleAndPermissionUsers::create([
    ...
    {
        $SuperAdminData = RoleAndPermissionUsers::where("role_type",  'super_admin')->get();
        if ($SuperAdminData) {
            return response(array("superAdminData" => $SuperAdminData), 200);
        } else {
            return response()->json(['message' => 'data not found']);
        }
    }
