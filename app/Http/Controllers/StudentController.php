...
class StudentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query = Student::where("admission_status", "admit");
        
            switch ($student_search_select) {
                case 'phone':
                case 'email':
                case 'id':
                case 'parents_id':
                    $query->where($student_search_select, $student_input_search);
                    break;
                case 'admission_date':
                    $query->whereRaw("STR_TO_DATE(admission_date, '%Y-%m-%d') BETWEEN ? AND ?", [$from_admission_date, $to_admission_date]);
                    break;
                case 'class':
                case 'first_name':
                case 'hostel_outi':
            ...
            if (!in_array($student_search_select, ['phone', 'email', 'id', 'parents_id'])) {
                $query->orderByRaw("FIELD(class, 'PG', 'NURSERY', 'LKG', 'UKG', '1ST', '2ND', '3RD'), roll_no");
            }
        
            $this->student_response = $query->paginate(100);
        ...
        try {
            ...
            if (count($this->response) != "0") {
                ...
                if (count($this->parent_response) != "0") {
                    ...
                    $this->TotalPayment = FeePayment::where('class_year', $student->class_year)->where('st_id', $student_id)->first() ?? 0;
    ...
    public function ManualUpdateStudent(Request $request){
       $st_id =  $request->st_id;
