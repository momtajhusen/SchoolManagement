    ...
    if(Student::where('id', $request->student_id)->where("admission_status", "admit")->first())
    {
      ...
      $roll = $Student->roll_no;
        ...
        PaymentHistory::create([
          ...
          'pay_date' => LaravelNepaliDate::from($payment_date)->toEnglishDate(),
          'processed_by' => session('user_name')
        ]);
    }
    else {
    ...
    try {
      if(Student::where('id', $request->student_id)->where("admission_status", "admit")->first())
      {
        ...
        // Start Payment History 
          ...
          $PaymentHistory->pay_date =  LaravelNepaliDate::from($payment_date)->toEnglishDate();
          $PaymentHistory->processed_by = session('user_name');
          $PaymentHistory->save();
        // Start Payment History 
   ...
   */
   ...
   public function BackyearManualDues(Request $request)
   {
       try {
           ...
           $payment_year = FeePayment::where('st_id', $st_id)
           ...
           if ($payment_year) {
               ...
               if (isset($free_fee)) {
                   $payment_year->free_fee = $free_fee == 0 ? '0' : $free_fee;
               }
               ...
               if (isset($free_fee)) {
                   $newPaymentYear->free_fee = $free_fee == 0 ? '0' : $free_fee;
               }
